...
function Draw( canvasID, brushImage ) {
...
Draw.prototype.onCanvasMouseDown = function () {
    var self = this;
...
Draw.prototype.onObjectMouseDown = function(){
    ...
    return function(event) {
        ...
        else{selectedIndex=-1;console.log("nothing selected");}
    }
}
...
Draw.prototype.onCanvasMouseMove = function () {
    var self = this;
    return function(event) {
        if(currentAction!="drawing"){
            self.canvas[0].width = self.canvas[0].width;
...
Draw.prototype.onCanvasMouseUp = function () {
    var self = this;
    return function(event) {
        $(document).unbind( self.mouseMoveEvent, self.mouseMoveHandler );
        $(document).unbind( self.mouseUpEvent, self.mouseUpHandler );
...
Draw.prototype.onExtraMouseMove = function(){
    var self = this;
    return function(event) {
        if(currentAction=="ellipse"){
            self.canvas[0].width = self.canvas[0].width;
...
Draw.prototype.onExtraMouseUp = function(){
    var self = this;
    return function(event) {
        $(document).unbind( self.mouseMoveEvent, self.mouseMoveHandler );
        $(document).unbind( self.mouseUpEvent, self.mouseUpHandler );
...
Draw.prototype.onExtraMouseDown = function(){
    var self = this;
    return function(event) {
...
Draw.prototype.updateMousePosition = function (event) {
  var target;
  if (this.touchSupported) {
...
Draw.prototype.drawPoint = function(point){
...
Draw.prototype.updateCanvasByDrawing = function (event) {
    ...
    if (currentAction=="drawing"){
        this.freehand.push(S2W(this.lastMousePoint));
    }
    this.context.lineTo( this.lastMousePoint.x, this.lastMousePoint.y );
    this.context.stroke();
}

Draw.prototype.updateCanvasByLine = function (event) {
    this.context.lineCap = 'round';
    this.context.strokeStyle=Colors.obj;
...
Draw.prototype.updateCanvasByCircle = function (event) {
    this.context.lineCap = 'round';
    this.context.strokeStyle=Colors.obj;
    var radius;
    this.drawPoint(this.center);
...
Draw.prototype.updateCanvasByEllipse = function (event) {
    this.context.lineCap = 'round';
    this.context.strokeStyle=Colors.obj;
...
Draw.prototype.updateCanvasBySine = function (event) {
    this.context.lineCap = 'round';
    this.context.strokeStyle=Colors.obj;
    this.drawPoint(this.center);
    this.context.beginPath();
...
Draw.prototype.updateCanvasByText = function (event) {
//    var editableText = $(
...
Draw.prototype.updateCanvasByBrush = function (event) {
...
Draw.prototype.toDataURL = function () {
